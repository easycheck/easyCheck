...
public class DatosCheckActivity extends Activity {
	...
	private String sugarComId = null;
		...
		btnConfirm.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (conId == null && !newContact) {
					alert.showAlertDialog(DatosCheckActivity.this, "Error",
							"No se ha seleccionado contacto", false);
				} else if (selectedType == null) {
					alert.showAlertDialog(DatosCheckActivity.this, "Error",
							"No se ha seleccionado tipo visita", false);
				} else {
	...
	protected void onResume() {
		...
		super.onResume();
		if (newContact) {
			spinner_adapter2 = new ArrayAdapter<String>(
					DatosCheckActivity.this,
			...
			runOnUiThread(new Runnable() {
				public void run() {
					...
					if (!placesListItems.isEmpty()) {
						...
						for (Iterator<HashMap<String, String>> iterator = placesListItems
					...
					// Log.d("ID", com);
					// Log.d("CONTAAAAAACTS",
					// placesListItems.get(0).get("name"));
					spinner_adapter2 = new ArrayAdapter<String>(
							DatosCheckActivity.this,
							android.R.layout.simple_spinner_item, categories2);
					spinner_adapter2
	...
	class DataTransaction extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
			...
			try {
				...
				// LLAMADA A SUGAR
				if (newContact) {
					conId = SugarConnection.newContact(id, contact.getName(),
							contact.getTelephone(), contact.getMail(),
							contact.getPosition(), sugarComId);
				}
				...
				Log.d("DESCRIPCION",
						((Editable) descriptor.getText()).toString());
				Log.d("FECHA", ((Editable) closingDate.getText()).toString());

				Log.d("Cantidad", ((Editable) quantity.getText()).toString());

				SugarConnection.newSale(id, ((Editable) descriptor.getText())
						.toString(), ((Editable) closingDate.getText())
						.toString(), selectedType, ((Editable) quantity
						.getText()).toString(), Utils
						.getStringFromDate(GregorianCalendar.getInstance()
								.getTime()), sugarComId, conId, id);
